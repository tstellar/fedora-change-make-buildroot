...
for f in rpm-specs/*; do
    ...
    case $f in
        rpm-specs/gap-pkg-circle.spec | rpm-specs/golang-github-pmezard-difflib.spec | rpm-specs/golang-github-segmentio-kafka.spec)
            continue
            ;;
        *)
            ;;
    ...
    if grep -l -q -e '\(^\|[^%]\)%make_build' -e '\(^\|[^%]\)%make_install' -e '\(^\|[^%]\)%__make' -e '\(^\|[^%]\)%{make_build}' -e '\(^\|[^%]\)%{make_install}' -e '\(^\|[^%]\)%{__make}' -e '^make ' -e '^DESTDIR.\+make' -e '^OPT.\+make' -e '^[^-#].\+make %{?_smp_mflags}' -e '\(^\|[^%]\)%makeinstall'  $f; then
...
if [ -e fail_nomake.txt ]; then
    ...
    cat uses_make.txt | sort | uniq | grep -v -x -F -f buildrequires_make.txt  > needs_br_make.txt
